...
One bar button that restarts the Omarchy shell — the one action that reliably closes a wedged popup
or panel.

## Why this exists

Third-party panels do not always dismiss when you click outside them, and a panel that has wedged
cannot be closed from its own UI. Restarting the shell tears every popup down. That normally means
...
```sh
omarchy plugin add https://github.com/thetimmyman/tmos-panic-close --enable
```

## Usage

Click the button in the bar. The shell restarts in about a second: the bar and every panel disappear
and come back. Nothing else on the machine restarts — no applications, no login session.

## Configure

```sh
omarchy bar move tmos.panic-close --section right
```

## Remove

```sh
omarchy plugin remove tmos.panic-close
```
